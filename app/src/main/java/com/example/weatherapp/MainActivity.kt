...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        apiCall = "https://api.openweathermap.org/data/2.5/weather?zip=${zipCode.toString()},us&appid=${apiKey}&units=metric"
        requestAPI()
    ...
    private suspend fun populateWeatherData(result: String) {
        withContext(Dispatchers.Main){
            ...
            updateTimeStamp.text = "Updated at: "+SimpleDateFormat("dd/MM/yyyy hh:mm a", Locale.ENGLISH)
                .format(Date(date.toLong() * 1000))
            weatherDes.text = "$weatherDescription"

            temp.text = "${String.format("%.0f",currentTemp.toDouble())}°C"
            lowTemp.text = "Low: ${String.format("%.1f",lowestTemp.toDouble())}°C"
            maxTemp.text = "High: ${String.format("%.1f",highestTemp.toDouble())}°C"
            sunrise.text = SimpleDateFormat("HH:mm a", Locale.ENGLISH).format(Date (sunriseTime.toLong()*1000))
            sunset.text = SimpleDateFormat("HH:mm a", Locale.ENGLISH).format(Date (sunsetTime.toLong()*1000))
